...
async def auto_revert_flash_sales(bot: Bot):
    """
    Background task: check for expired flash sales every 60 seconds,
    revert category prices, and notify the primary admin.
    """
    import os
    admin_id_raw = os.getenv("ADMIN_ID", "")
    primary_admin = admin_id_raw.split(",")[0].strip() if admin_id_raw else None
    ...
    while True:
        try:
            ...
            from database.requests import check_expired_flash_sales
            expired_categories = await check_expired_flash_sales(db)

            if expired_categories and primary_admin:
                for cat in expired_categories:
                    try:
                        end_msg = (
                            f"⏰ <b>FLASH SALE EXPIRED</b>\n"
                            f"━━━━━━━━━━━━━━━━━━\n\n"
                            f"📂 <b>Category:</b> {cat.name}\n"
                            f"💰 Price reverted back to: <b>₹{cat.get('flash_sale_original_price')}</b>\n\n"
                            f"⚡ <i>Normal sales pricing has resumed.</i>"
                        )
                        await bot.send_message(primary_admin, end_msg)
                    except Exception:
                        pass

        except Exception as e:
            print(f"❌ [Auto-Revert Flash Sale Error] {e}")
