...
async def add_category(db: AsyncIOMotorDatabase, name: str,
                       description: str = None, terms: str = None, instructions: str = None, qr_file_id: str = None) -> Doc:
    from database.db import get_next_id
    cat_id = await get_next_id("categories")
    doc = {"_id": cat_id, "name": name, "description": description,
           "terms": terms, "instructions": instructions, "qr_file_id": qr_file_id, "is_active": True}
...
async def update_category_instructions(db: AsyncIOMotorDatabase, category_id: int, instructions: str):
    await db.categories.update_one({"_id": category_id}, {"$set": {"instructions": instructions}})


async def update_category_qr(db: AsyncIOMotorDatabase, category_id: int, qr_file_id: str):
    await db.categories.update_one({"_id": category_id}, {"$set": {"qr_file_id": qr_file_id}})
...
async def reverse_sale(db: AsyncIOMotorDatabase, tx_id: int):
    """Reverse a completed sale — marks coupons unsold and transaction as failed."""
    await asyncio.gather(
        db.coupons.update_many(
            {"transaction_id": tx_id},
            {"$set": {"is_sold": False, "sold_to": None, "transaction_id": None}},
        ),
        db.transactions.update_one({"_id": tx_id}, {"$set": {"status": "reversed"}}),
    )
...
async def update_user(db: AsyncIOMotorDatabase, user_id: int, **kwargs):
    if user_id in _get_admin_ids():
        kwargs.pop("is_blocked", None)
        kwargs.pop("is_suspicious", None)
        kwargs.pop("warning_count", None)
        if not kwargs:
            return
...
async def get_setting(db: AsyncIOMotorDatabase, key: str, default: str = None) -> Optional[str]:
    ...
    return doc.get("value", default) if doc is not None else default
...
async def start_flash_sale(db: AsyncIOMotorDatabase, cat_id: int, promo_price: float, duration_mins: int) -> Tuple[bool, float]:
    """
    Start a Flash Sale for a category.
    Returns (success, original_price).
    """
    cat = await db.categories.find_one({"_id": cat_id})
    if not cat:
        return False, 0.0

    # Get original price if not already in a flash sale
    original_price = cat.get("flash_sale_original_price")
    if original_price is None:
        original_price = await get_category_price(db, cat_id)
        if original_price is None:
            original_price = promo_price  # Fallback if no coupons exist

    ends_at = datetime.utcnow() + timedelta(minutes=duration_mins)
    ...
    await asyncio.gather(
        db.categories.update_one(
            ...
            {"$set": {
                ...
                "flash_sale_price": promo_price,
                "flash_sale_original_price": original_price,
                "flash_sale_ends_at": ends_at
            }}
        ),
        db.coupons.update_many(
            {"category_id": cat_id, "is_sold": False},
            {"$set": {"price_inr": promo_price}}
        )
    )
    return True, original_price


async def stop_flash_sale(db: AsyncIOMotorDatabase, cat_id: int) -> bool:
    """
    Stop/revert an active Flash Sale.
    Returns True if successfully stopped.
    """
    cat = await db.categories.find_one({"_id": cat_id})
    if not cat or not cat.get("flash_sale_active"):
        return False

    original_price = cat.get("flash_sale_original_price", 0.0)
    ...
    await asyncio.gather(
        db.categories.update_one(
            ...
            {"$set": {
                ...
                "flash_sale_original_price": None,
                "flash_sale_ends_at": None
            }}
        ),
        db.coupons.update_many(
            {"category_id": cat_id, "is_sold": False},
            {"$set": {"price_inr": original_price}}
        )
    )
    return True


async def check_expired_flash_sales(db: AsyncIOMotorDatabase) -> list:
    """
    Find all active flash sales that have expired and stop them.
    Returns list of category Docs that expired.
    """
    now = datetime.utcnow()
    expired_cats = await db.categories.find({
        "flash_sale_active": True,
        "flash_sale_ends_at": {"$lte": now}
    }).to_list(None)

    for cat in expired_cats:
        await stop_flash_sale(db, cat["_id"])

    return _dl(expired_cats)
