...
from utils.fonts import (bold_sans, sans_normal, stylize_html,
    ...
    price_tag, code_block, order_id_display, countdown_text, escape_html)
...
from database.requests import (
    ...
    finalize_sale, reverse_sale, start_flash_sale, stop_flash_sale, update_transaction, get_all_users, get_stats, 
...
class AdminStates(StatesGroup):
    ...
    edit_cat_instr = State()
    edit_cat_qr = State()
    edit_welcome_msg = State()
    add_flash_price = State()
    add_flash_duration = State()

# Stylized Button Names
BTN_STATS = f"📊 {bold_sans('Statistics')}"
BTN_ALL_VOUCHERS = f"🎫 {bold_sans('All Vouchers')}"
BTN_MANAGE_USERS = f"👥 {bold_sans('Manage Users')}"
BTN_ORDERS_DASHBOARD = f"📦 {bold_sans('Orders Dashboard')}"
BTN_BROADCAST = f"📣 {bold_sans('Broadcast')}"
BTN_LOOKUP_ORDER = f"🔍 {bold_sans('Lookup Order')}"
BTN_BOT_SETTINGS = f"⚙️ {bold_sans('Bot Settings')}"
BTN_EXIT_ADMIN = f"🚪 {bold_sans('Exit Admin')}"
BTN_REFERRAL_EVENT = f"🎁 {bold_sans('Referral Event')}"

def BTN_PENDING_ORDERS(pending_count: int = 0) -> str:
    pending_text = f" ({pending_count})" if pending_count > 0 else ""
    return f"⏳ {bold_sans(f'Pending Orders{pending_text}')}"

def admin_main_keyboard(pending_count: int = 0):
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text=BTN_STATS), KeyboardButton(text=BTN_ALL_VOUCHERS)],
        [KeyboardButton(text=BTN_MANAGE_USERS), KeyboardButton(text=BTN_ORDERS_DASHBOARD)],
        [KeyboardButton(text=BTN_PENDING_ORDERS(pending_count)), KeyboardButton(text=BTN_BROADCAST)],
        [KeyboardButton(text=BTN_LOOKUP_ORDER), KeyboardButton(text=BTN_BOT_SETTINGS)],
        [KeyboardButton(text=BTN_EXIT_ADMIN)]
    ], resize_keyboard=True)

def is_menu_button(text: str):
    if not text or not isinstance(text, str): return False
    if text.startswith(f"⏳ {bold_sans('Pending Orders')}"): return True
    buttons = [BTN_STATS, BTN_ALL_VOUCHERS, BTN_MANAGE_USERS, BTN_ORDERS_DASHBOARD, BTN_BROADCAST, BTN_LOOKUP_ORDER, BTN_BOT_SETTINGS, BTN_EXIT_ADMIN]
...
async def cmd_admin(message: Message, state: FSMContext, session: AsyncIOMotorDatabase):
    ...
    await message.answer(sh("🛠️ <b>Admin Control Panel</b>"), reply_markup=admin_main_keyboard(pending))
...
async def cancel_handler(message: Message, state: FSMContext, session: AsyncIOMotorDatabase):
    ...
    stats = await get_order_dashboard_stats(session)
    pending = stats["pending"]
    await message.answer(sh("❌ <b>Cancelled.</b>"), reply_markup=admin_main_keyboard(pending))

# --- Settings ---
@router.message(F.text == BTN_BOT_SETTINGS)
async def admin_settings(message: Message, session: AsyncIOMotorDatabase, state: FSMContext = None):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=bold_sans(f"🛠️ Maintenance: {m_status}"), callback_data="toggle_m_mode")],
        [InlineKeyboardButton(text=bold_sans("📢 Channel Manager"), callback_data="admin_channels")],
        [InlineKeyboardButton(text=bold_sans("🎁 Referral Event"), callback_data="admin_rewards_main_cb")],
        [InlineKeyboardButton(text=bold_sans(f"🎁 Referral System: {r_status}"), callback_data="toggle_r_mode")],
        [InlineKeyboardButton(text=bold_sans(f"🎯 Set Ref Goal: {r_goal}"), callback_data="admin_set_ref_goal")],
        [InlineKeyboardButton(text=bold_sans("📝 Edit Welcome Msg"), callback_data="admin_edit_welcome_msg")],
        [InlineKeyboardButton(text=bold_sans("📞 Support Settings"), callback_data="admin_support_settings")],
        [InlineKeyboardButton(text=bold_sans("🚫 Blocked Users"), callback_data="admin_blocked_list")],
        [InlineKeyboardButton(text=bold_sans("🔑 Admin Manager"), callback_data="admin_list_managers")],
        [InlineKeyboardButton(text=bold_sans("🔙 Close"), callback_data="admin_close_settings")]
    ])
    await message.answer(sh("⚙️ <b>Advanced Bot Settings</b>"), reply_markup=keyboard)

@router.callback_query(F.data == "admin_edit_welcome_msg")
async def edit_welcome_msg_start(callback: CallbackQuery, state: FSMContext, session: AsyncIOMotorDatabase):
    await state.set_state(AdminStates.edit_welcome_msg)
    from handlers.user import DEFAULT_WELCOME_TEXT
    current = await get_setting(session, "welcome_message", DEFAULT_WELCOME_TEXT)
    
    msg = (
        sh("📝 <b>EDIT WELCOME MESSAGE</b>\n") +
        sh("━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n") +
        sb("This message is shown to users when they run the /start command.\n\n") +
        f"📄 <b>Current Message:</b>\n<pre>{html.escape(current)}</pre>\n\n" +
        sb("👉 Please send the new welcome message in HTML format. You can use standard formatting tags like &lt;b&gt;, &lt;i&gt;, &lt;code&gt;, &lt;u&gt;, etc.\n") +
        sb("📌 <i>Use {full_name} to insert the user's name dynamically.</i>\n\n") +
        sb("<i>Type '/cancel' or use another menu button to cancel.</i>")
    )
    await callback.message.answer(msg)
    await callback.answer()

@router.message(AdminStates.edit_welcome_msg)
async def edit_welcome_msg_proc(message: Message, state: FSMContext, session: AsyncIOMotorDatabase):
    if message.text and (is_menu_button(message.text) or message.text.casefold() == "/cancel"):
        await state.clear()
        await message.answer("🚪 Canceled editing welcome message.")
        return
    
    from utils.fonts import sanitize_telegram_html
    new_welcome = sanitize_telegram_html(message.text.strip())
    if not new_welcome:
        await message.answer("❌ Message cannot be empty. Please send some text or /cancel.")
        return
        
    await set_setting(session, "welcome_message", new_welcome)
    await state.clear()
    
    success_msg = (
        sh("✅ <b>WELCOME MESSAGE UPDATED!</b>\n") +
        sh("━━━━━━━━━━━━━━━━━━━━\n\n") +
        sb("The new welcome message has been saved and will be shown to users who use the /start command.")
    )
...
@router.message(F.text.startswith(f"⏳ {bold_sans('Pending Orders')}"))
...
# --- Referral Rewards (Event) ---
@router.message(F.text == BTN_REFERRAL_EVENT)
...
# --- Voucher Management ---
@router.message(F.text == BTN_ALL_VOUCHERS)
...
async def add_category_desc_proc(message: Message, state: FSMContext):
    if is_menu_button(message.text): await state.clear(); return
    from utils.fonts import sanitize_telegram_html
    desc = None if message.text.strip() == '-' else sanitize_telegram_html(message.text)
...
async def add_category_terms_proc(message: Message, state: FSMContext, session: AsyncIOMotorDatabase):
    if is_menu_button(message.text): await state.clear(); return
    from utils.fonts import sanitize_telegram_html
    terms = None if message.text.strip() == '-' else sanitize_telegram_html(message.text)
...
async def manage_cat(callback: CallbackQuery, session: AsyncIOMotorDatabase):
    ...
    if cat.get("flash_sale_active"):
        ends_at = cat.get("flash_sale_ends_at")
        rem_mins = 0
        if ends_at:
            rem = ends_at - datetime.utcnow()
            rem_mins = max(0, int(rem.total_seconds() / 60))
        flash_text = f"\n⚡ <b>Flash Sale Active:</b> ₹{cat.get('flash_sale_price')} (Ends in {rem_mins} mins)\n"
        flash_btn_text = "⚡ Flash Sale (Active)"
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(text="💰 Edit Price", callback_data=f"admin_edit_price_{cat_id}"), InlineKeyboardButton(text=flash_btn_text, callback_data=f"admin_flash_sale_{cat_id}")],
        [InlineKeyboardButton(text="📝 Edit Name", callback_data=f"admin_edit_name_{cat_id}"), InlineKeyboardButton(text="📖 Edit Desc", callback_data=f"admin_edit_desc_{cat_id}")],
        [InlineKeyboardButton(text="📜 Edit Terms", callback_data=f"admin_edit_terms_{cat_id}"), InlineKeyboardButton(text="🔑 Edit Instructions", callback_data=f"admin_edit_instr_{cat_id}")],
        [InlineKeyboardButton(text="🖼️ Edit QR Code", callback_data=f"admin_edit_qr_{cat_id}")],
    ...
    text = (
        ...
        f"📊 Status: <b>{status_text}</b>{flash_text}\n"
...
@router.callback_query(F.data.startswith("admin_flash_sale_"))
async def admin_flash_sale_cb(callback: CallbackQuery, state: FSMContext, session: AsyncIOMotorDatabase):
    cat_id = int(callback.data.split("_")[-1])
    cat = await get_category(session, cat_id)
    if not cat:
    ...
    if cat.get("flash_sale_active"):
        ...
        text = (
            f"⚡ <b>FLASH SALE DETAILS</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n\n"
            f"📂 <b>Category:</b> {cat.name}\n"
            f"🔥 <b>Flash Price:</b> ₹{cat.get('flash_sale_price')}\n"
            f"💰 Original Price: ₹{cat.get('flash_sale_original_price')}\n"
            f"⏱ <b>Ends In:</b> {rem_mins} minutes\n\n"
            f"<i>Would you like to stop this Flash Sale immediately and revert the price back to normal?</i>"
        )
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🛑 STOP FLASH SALE", callback_data=f"admin_flash_stop_{cat_id}")],
            [InlineKeyboardButton(text="🔙 Back", callback_data=f"admin_manage_cat_{cat_id}")]
        ])
        await callback.message.edit_text(text, reply_markup=kb)
    else:
        curr_price = await get_category_price(session, cat_id) or 0.0
        await state.update_data(flash_cat_id=cat_id, flash_cat_name=cat.name, flash_orig_price=curr_price)
        await state.set_state(AdminStates.add_flash_price)
        await callback.message.answer(
            f"⚡ <b>Start Flash Sale: {cat.name}</b>\n\n"
            f"💰 Current Price: ₹{curr_price}\n\n"
            f"<i>Please enter the promotional price (INR) for the flash sale (or type '/cancel'):</i>"
        )
    await callback.answer()


@router.message(AdminStates.add_flash_price)
async def add_flash_price_proc(message: Message, state: FSMContext):
    if is_menu_button(message.text): await state.clear(); return
    try:
        price = float(message.text.strip())
        if price <= 0: raise ValueError
    except:
        return await message.answer("❌ Invalid price. Enter a positive number (e.g. <b>19.99</b>):")
    
    await state.update_data(flash_promo_price=price)
    await state.set_state(AdminStates.add_flash_duration)
    await message.answer(
        f"⚡ <b>Flash Sale Price:</b> ₹{price}\n\n"
        f"<i>Enter sale duration in minutes (e.g. 60 for 1 hour, or type '/cancel'):</i>"
    )


@router.message(AdminStates.add_flash_duration)
async def add_flash_duration_proc(message: Message, state: FSMContext, session: AsyncIOMotorDatabase):
    if is_menu_button(message.text): await state.clear(); return
    try:
        duration = int(message.text.strip())
        if duration < 1: raise ValueError
    except:
        return await message.answer("❌ Invalid duration. Enter an integer greater than 0 (number of minutes):")
    
    data = await state.get_data()
    cat_id = data.get('flash_cat_id')
    promo_price = data.get('flash_promo_price')
    cat_name = data.get('flash_cat_name')
    
    if not cat_id or promo_price is None:
        await state.clear()
        return await message.answer("❌ Error: Process timed out. Please try again.")

    success, orig = await start_flash_sale(session, cat_id, promo_price, duration)
    await state.clear()
    
    if not success:
        return await message.answer("❌ Error starting flash sale. Please check category settings.")
        
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📢 Notify All Users (Broadcast)", callback_data=f"admin_flash_notify_{cat_id}_{promo_price}_{duration}")],
        [InlineKeyboardButton(text="✅ Done", callback_data=f"admin_manage_cat_{cat_id}")]
    ])
    
    await message.answer(
        f"⚡ <b>Flash Sale Started successfully!</b>\n━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📂 Category: <b>{cat_name}</b>\n"
        f"💰 Promo Price: <b>₹{promo_price}</b> (reverts to ₹{orig})\n"
        f"⏱ Duration: <b>{duration} minutes</b>\n\n"
        f"<i>Would you like to send an automated broadcast message to notify all users?</i>",
        reply_markup=kb
    )


@router.callback_query(F.data.startswith("admin_flash_stop_"))
async def admin_flash_stop_cb(callback: CallbackQuery, session: AsyncIOMotorDatabase):
    cat_id = int(callback.data.split("_")[-1])
    cat = await get_category(session, cat_id)
    orig_price = cat.get("flash_sale_original_price", 0.0) if cat else 0.0
    
    success = await stop_flash_sale(session, cat_id)
    if success:
        await callback.message.edit_text(
            f"✅ <b>Flash Sale Stopped!</b>\n\n"
            f"Category pricing has reverted back to its original price of <b>₹{orig_price}</b>.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 Back to Category", callback_data=f"admin_manage_cat_{cat_id}")]
            ])
        )
    else:
        await callback.answer("❌ Flash Sale is not active or could not be stopped.", show_alert=True)
    await callback.answer()


@router.callback_query(F.data.startswith("admin_flash_notify_"))
async def admin_flash_notify_cb(callback: CallbackQuery, bot: Bot, session: AsyncIOMotorDatabase):
    parts = callback.data.split("_")
    cat_id = int(parts[3])
    promo_price = float(parts[4])
    ...
    await callback.message.edit_text("⏳ Broadcasting Flash Sale alert to all users...")
    
    from utils.fonts import header, stat_line, stat_line_bold, divider, footer_box
    
    broadcast_text = (
        header("⚡", "MEGA FLASH SALE") +
        sb("A limited-time flash sale has started! Get coupons at huge discounts! 🛒🔥\n\n") +
        stat_line("📂", "Category", cat.name) +
        stat_line_bold("💸", "Flash Price", f"₹{promo_price}") +
        stat_line("⏱", "Duration", f"{duration} minutes") +
        divider() +
        footer_box("Tap '🛒 Buy Coupons' in the main menu now before stock runs out!")
    )
    ...
    for u in users:
        try:
            await bot.send_message(chat_id=u.id, text=broadcast_text)
            count += 1
            await asyncio.sleep(0.05)
        except Exception:
            pass
            
    await callback.message.answer(f"✅ <b>Flash Sale Broadcast Complete!</b>\n\nSuccessfully sent to <b>{count}</b> users.")
    await callback.message.delete()
    await callback.answer()
...
async def edit_terms_proc(message: Message, state: FSMContext):
    ...
    from utils.fonts import sanitize_telegram_html
    sanitized = sanitize_telegram_html(message.text)
    await state.update_data(new_terms=sanitized)
...
@router.callback_query(F.data.startswith("admin_edit_instr_"), ~F.data.endswith("_exec"))
async def edit_instructions_start(callback: CallbackQuery, state: FSMContext, session: AsyncIOMotorDatabase):
    cat_id = int(callback.data.split("_")[-1])
    cat = await get_category(session, cat_id)
    if not cat:
        return await callback.answer("❌ Category not found.", show_alert=True)
        
    await state.set_state(AdminStates.edit_cat_instr)
    await state.update_data(edit_cat_id=cat_id)
    
    from handlers.user import DEFAULT_INSTRUCTIONS
    current = cat.instructions or DEFAULT_INSTRUCTIONS
    
    msg = (
        sh("📝 <b>EDIT REDEMPTION INSTRUCTIONS</b>\n") +
        sh(f"Category: <b>{cat.name}</b>\n") +
        sh("━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n") +
        sb("These instructions are shown to the user when they view their coupon codes for this category.\n\n") +
        f"📄 <b>Current Instructions:</b>\n<pre>{html.escape(current)}</pre>\n\n" +
        sb("👉 Please send the new instructions in HTML format. You can use standard formatting tags.\n\n") +
        sb("<i>Type '/cancel' to abort.</i>")
    )
    await callback.message.answer(msg)
    await callback.answer()

@router.message(AdminStates.edit_cat_instr)
async def edit_instructions_proc(message: Message, state: FSMContext):
    if is_menu_button(message.text) or message.text.casefold() == "/cancel":
        await state.clear()
        await message.answer("🚪 Canceled editing instructions.")
        return
        
    from utils.fonts import sanitize_telegram_html
    new_instructions = sanitize_telegram_html(message.text.strip())
    if not new_instructions:
        await message.answer("❌ Instructions cannot be empty. Please send some text or /cancel.")
        return
        
    data = await state.get_data()
    cat_id = data.get('edit_cat_id')
    
    await state.update_data(new_instructions=new_instructions)
    text = (
        sh("📝 <b>Confirm Instructions Update</b>\n\n") +
        sb(f"New Instructions:\n<pre>{html.escape(new_instructions)}</pre>\n\n") +
        sb("Are you sure you want to apply this?")
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ CONFIRM", callback_data="admin_edit_instr_exec")],
        [InlineKeyboardButton(text="❌ CANCEL", callback_data=f"admin_manage_cat_{cat_id}")]
    ])
    await message.answer(text, reply_markup=kb)

@router.callback_query(F.data == "admin_edit_instr_exec")
async def edit_instructions_exec(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    cat_id = data.get('edit_cat_id')
    new_instructions = data.get('new_instructions')
    
    if not cat_id or not new_instructions:
        return await callback.answer("❌ Error: State lost. Please start over.", show_alert=True)
        
    async with AsyncSessionLocal() as session:
        from database.requests import update_category_instructions
        await update_category_instructions(session, cat_id, new_instructions)
    ...
    await callback.message.edit_text("✅ Category redemption instructions updated!", reply_markup=kb)
    await callback.answer()

@router.callback_query(F.data.startswith("admin_edit_qr_"), ~F.data.endswith("_exec"))
async def edit_qr_start(callback: CallbackQuery, state: FSMContext, session: AsyncIOMotorDatabase):
    cat_id = int(callback.data.split("_")[-1])
    cat = await get_category(session, cat_id)
    if not cat:
        return await callback.answer("❌ Category not found.", show_alert=True)
        
    await state.set_state(AdminStates.edit_cat_qr)
    await state.update_data(edit_cat_id=cat_id)
    
    msg = (
        sh("🖼️ <b>UPDATE CATEGORY QR CODE</b>\n") +
        sh(f"Category: <b>{cat.name}</b>\n") +
        sh("━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n") +
        sb("This QR code will be displayed to users when they checkout this category.\n\n") +
        (f"✅ Current: <i>Custom QR set</i>\n\n" if cat.qr_file_id else "⚠️ Current: <i>Using default bot QR</i>\n\n") +
        sb("👉 Please upload/send the new QR code image now.\n\n") +
        sb("<i>Type '/cancel' or use another menu button to cancel.</i>")
    )
    
    kb = []
    if cat.qr_file_id:
        kb.append([InlineKeyboardButton(text="❌ Remove Custom QR", callback_data=f"admin_remove_qr_{cat_id}")])
    kb.append([InlineKeyboardButton(text="❌ Cancel", callback_data=f"admin_manage_cat_{cat_id}")])
    
    if cat.qr_file_id:
        try:
            await callback.message.answer_photo(photo=cat.qr_file_id, caption=msg, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
        except Exception:
            await callback.message.answer(msg, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
    else:
        await callback.message.answer(msg, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
        
    await callback.answer()

@router.message(AdminStates.edit_cat_qr)
async def edit_qr_proc(message: Message, state: FSMContext):
    if message.text and (is_menu_button(message.text) or message.text.casefold() == "/cancel"):
        await state.clear()
        await message.answer("🚪 Canceled updating QR Code.")
        return
        
    if not message.photo:
        await message.answer("❌ Please upload a photo/image of the QR code, or send /cancel.")
        return
    ...
    await state.update_data(new_qr_file_id=photo_file_id)
    
    text = (
        sh("🖼️ <b>Confirm QR Code Update</b>\n\n") +
        sb("Are you sure you want to set this image as the payment QR code for this category?")
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ CONFIRM", callback_data="admin_edit_qr_exec")],
        [InlineKeyboardButton(text="❌ CANCEL", callback_data=f"admin_manage_cat_{cat_id}")]
    ])
    await message.answer_photo(photo=photo_file_id, caption=text, reply_markup=kb)

@router.callback_query(F.data == "admin_edit_qr_exec")
async def edit_qr_exec(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    cat_id = data.get('edit_cat_id')
    new_qr_file_id = data.get('new_qr_file_id')
    
    if not cat_id or not new_qr_file_id:
        return await callback.answer("❌ Error: State lost. Please start over.", show_alert=True)
        
    async with AsyncSessionLocal() as session:
        from database.requests import update_category_qr
        await update_category_qr(session, cat_id, new_qr_file_id)
    ...
    try:
        ...
        pass
    await callback.message.answer("✅ Category payment QR code updated successfully!", reply_markup=kb)
    await callback.answer()

@router.callback_query(F.data.startswith("admin_remove_qr_"))
async def remove_qr_exec(callback: CallbackQuery, session: AsyncIOMotorDatabase):
    cat_id = int(callback.data.split("_")[-1])
    from database.requests import update_category_qr
    await update_category_qr(session, cat_id, None)
    ...
    try:
        ...
        pass
    await callback.message.answer("✅ Category payment QR code reset to default!", reply_markup=kb)
    await callback.answer()
...
async def admin_list_managers_cb(callback: CallbackQuery, session: AsyncIOMotorDatabase):
    ...
    if not admins:
        ...
        for a in admins:
            text += f"▪️ {html.escape(a.full_name)} (<code>{a.id}</code>)\n"
...
async def search_user_proc(message: Message, state: FSMContext, session: AsyncIOMotorDatabase):
    ...
    text = f"👤 <b>{html.escape(user.full_name)}</b>\nStatus: {status}\n{warn_text}\nRef: {user.referral_count}\n🆔 ID: <code>{user.id}</code>\n\n🛍 <b>Purchases:</b>\n"
...
async def admin_toggle_admin_cb(callback: CallbackQuery, state: FSMContext, session: AsyncIOMotorDatabase):
    ...
    # Evict from in-memory cache to apply immediately
    from middlewares.checks import USER_CACHE
    USER_CACHE.pop(u_id, None)
...
async def view_users_list_cb(callback: CallbackQuery, state: FSMContext, session: AsyncIOMotorDatabase):
    ...
    text = "👥 <b>Users Log:</b>\n\n"
    for u in users[:20]: text += f"▪️ {html.escape(u.full_name)} | ID: <code>{u.id}</code> | Buy: {u.bought}\n"
...
async def suspicious_users_cb(callback: CallbackQuery, state: FSMContext, session: AsyncIOMotorDatabase):
    ...
    if not users:
        ...
        for u in users[:20]:
            ...
            text += f"▪️ <b>{html.escape(u.full_name)}</b> (<code>{u.id}</code>) | Warns: <b>{u.warning_count}/5</b> | {status}\n"
...
@router.message(F.text == BTN_LOOKUP_ORDER)
...
@router.message(F.text == BTN_ORDERS_DASHBOARD)
async def admin_orders_dashboard(message: Message, state: FSMContext, session: AsyncIOMotorDatabase):
    ...
    text = (
        ...
        sub_header("📅", "ACTIVITY SUMMARY") +
        stat_line("☀️", "Orders Today", str(today_count)) +
        stat_line("🌙", "Orders Yesterday", str(yesterday_count)) +
        "\n" +
        sub_header("🚦", "STATUS BREAKDOWN") +
        stat_line("⏳", "Pending", str(pending)) +
        stat_line("✅", "Completed", str(completed)) +
        stat_line("❌", "User Errors", str(failed)) +
        stat_line("⏰", "Timeouts", str(cancelled)) +
        divider() +
        footer_box("Manage pending orders and search invoices below:")
    )
...
async def send_order_details(message_obj: Message, tx_id: int, session: AsyncIOMotorDatabase, is_callback: bool = False):
    ...
    status_text = "PENDING" if tx.status == "pending" else "COMPLETED" if tx.status == "completed" else "REJECTED"
    
    text = (
        header("📋", "ORDER DETAILS") +
        stat_line("👤", "User", html.escape(user.full_name) if user else 'Unknown') +
        stat_line("🆔", "User ID", f"<code>{tx.user_id}</code>") +
        stat_line("📅", "Date", tx.created_at.strftime('%d %b %Y, %H:%M:%S')) +
        divider() +
        stat_line("🧾", "Order ID", f"<code>{tx.provider_payment_charge_id}</code>") +
        stat_line_bold("💸", "Amount", f"₹{tx.amount}") +
        stat_line("💳", "Proof/UTR", f"<code>{tx.payment_proof_id or 'None'}</code>") +
        stat_line("🚦", "Status", f"{status_emoji} {status_text}") +
        divider() +
        sub_header("🎫", "COUPONS INVOLVED")
    )
    ...
    if tx.status == 'pending':
        ...
        kb_list.append([InlineKeyboardButton(text="⚠️ Reverse Sale", callback_data=f"admin_reverse_{tx.id}")])
...
async def approve(callback: CallbackQuery, bot: Bot, session: AsyncIOMotorDatabase):
    ...
    notification_msg = (
        ...
        stat_line_bold("💰", "Total Paid", f"₹{tx.amount}") +
        divider() +
        sb("Your payment has been verified successfully. Your coupon codes are ready!\n\n") +
        sb("👇 <b>Click the button below to view your code(s):</b>")
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=bold_sans("🔑 View Coupon Codes"), callback_data=f"show_codes_{tx.id}")]
    ])
    await bot.send_message(user_id, notification_msg, reply_markup=kb, disable_web_page_preview=True)
    
    # Notify Admin / Update Admin Message in chat
    user = await get_user_by_id(session, user_id)
    client_name = html.escape(user.full_name) if user else "Unknown"
    cat_name = items[0][1].name if items else "(Voucher)"
    
    admin_msg = (
        header("✅", "ORDER APPROVED & COMPLETED") +
        stat_line("👤", "Client", client_name) +
        stat_line("🆔", "User ID", f"<code>{user_id}</code>") +
        stat_line("📦", "Item", f"{tx.quantity} × {cat_name}") +
        stat_line_bold("💸", "Paid", f"₹{tx.amount}") +
        divider() +
        stat_line("📋", "Order ID", f"<code>{tx_charge_id}</code>") +
        stat_line("🔑", "UTR", f"<code>{tx.utr or 'N/A'}</code>") +
        divider() +
        footer_box("Successfully verified, approved & delivered!")
    )
    
    try:
        await callback.message.edit_caption(caption=admin_msg, reply_markup=None)
    except Exception:
        try:
            await callback.message.edit_text(text=admin_msg, reply_markup=None)
        except Exception:
            pass
            
    # Send a proper success notification message to the primary admin
    import os
    admin_id_raw = os.getenv("ADMIN_ID", "")
    primary_admin = int(admin_id_raw.split(",")[0].strip()) if admin_id_raw else None
    if primary_admin:
        try:
            success_notification = (
                header("🔔", "ORDER COMPLETED") +
                sb(f"Admin <b>{html.escape(callback.from_user.full_name)}</b> verified and approved the payment!\n\n") +
                stat_line("👤", "Client", client_name) +
                stat_line("🆔", "User ID", f"<code>{user_id}</code>") +
                stat_line("📦", "Item", f"{tx.quantity} × {cat_name}") +
                stat_line_bold("💸", "Paid", f"₹{tx.amount}") +
                ...
                stat_line("🔑", "UTR", f"<code>{tx.utr or 'N/A'}</code>") +
                divider() +
                footer_box("Status: Successfully Completed & Delivered ✅")
            )
            await bot.send_message(chat_id=primary_admin, text=success_notification)
        except Exception as e:
            print(f"Error sending success notification: {e}")
            
    await callback.answer("✅ Approved & Delivered!")
...
async def reject(callback: CallbackQuery, bot: Bot, session: AsyncIOMotorDatabase):
    ...
    admin_msg = (
        ...
        stat_line("🔑", "UTR", f"<code>{tx.utr or 'N/A'}</code>") +
        divider() +
        footer_box("Rejected. User has been notified.")
    )
...
@router.callback_query(F.data.startswith("admin_reverse_"))
async def reverse_sale_handler(callback: CallbackQuery, bot: Bot, session: AsyncIOMotorDatabase):
    """Reverse an auto-approved sale in case of fraud."""
    tx_id = int(callback.data.split("_")[-1])
    tx = await get_transaction(session, tx_id)
    if not tx or tx.status not in ('completed',):
        return await callback.answer("❌ Cannot reverse — order is not in completed state.", show_alert=True)
    
    user_id = tx.user_id
    tx_charge_id = tx.provider_payment_charge_id or f"TXN-{tx.id}"
    
    # Reverse the sale in DB
    await reverse_sale(session, tx_id)
    
    # Block the user
    from database.requests import update_user
    await update_user(session, user_id, is_blocked=True, is_suspicious=True)
    from middlewares.checks import USER_CACHE
    ...
    try:
        await bot.send_message(
            user_id,
            f"🚨 <b>ORDER REVERSED</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n\n"
            f"📋 <b>Order:</b> <code>{tx_charge_id}</code>\n\n"
            f"Your order has been reversed due to a payment issue. "
    ...
    try:
        await callback.message.edit_caption(
            caption=(
                f"⚠️ <b>SALE REVERSED</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━\n\n"
                f"📋 Order: <code>{tx_charge_id}</code>\n"
                f"👤 User: <code>{user_id}</code>\n\n"
                f"✅ Coupons released back to stock\n"
                f"🚫 User blocked & flagged"
            )
        )
    except Exception:
        try:
            await callback.message.edit_text(
                f"⚠️ <b>SALE REVERSED</b> — Order <code>{tx_charge_id}</code>\n"
                f"Coupons released. User <code>{user_id}</code> blocked."
            )
        except Exception:
            pass
    
    await callback.answer("⚠️ Sale reversed. User blocked.", show_alert=True)
...
async def proc_chan_link(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        from middlewares.checks import GLOBAL_CACHE
        GLOBAL_CACHE["channels"]["expires"] = 0
...
async def blocked_users_cb(callback: CallbackQuery, state: FSMContext):
    ...
    if not users:
        ...
        for u in users[:20]:
            text += f"▪️ <b>{html.escape(u.full_name)}</b> (<code>{u.id}</code>)\n"
