...
class GlobalCheckMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: Message | CallbackQuery, data: dict):
        ...
        # ── 0. FSM Auto-Clear for Navigation ──────────────────────────────────
        if isinstance(event, Message) and event.text:
            text = event.text.strip()
            is_cmd_or_menu = text.startswith("/")
            if not is_cmd_or_menu:
                try:
                    from handlers.user import MENU_BUTTONS
            ...
            if not is_cmd_or_menu:
                try:
                    from handlers.admin import is_menu_button
    ...
    async def _check(ch):
        ...
        if isinstance(chat_id_val, str):
            ...
            if (v.startswith('-') and v[1:].isdigit()) or v.isdigit():
                chat_id_val = int(v)
        try:
            m = await bot.get_chat_member(chat_id=chat_id_val, user_id=user_id)
            if m.status not in ("member", "administrator", "creator"):
                return ch
        except Exception as e:
            print(f"DEBUG: get_chat_member check failed for channel {chat_id_val} (user {user_id}): {e}")
            pass
        return None
